...
学习者学完某个域后会要求出练习题，按以下规范执行：

**位置**：在对应域文件夹下新建 `练习/` 子目录，放两个文件：
- `题目.ipynb` —— 每道题：一个 markdown cell（题目说明）+ 一个 code cell（只写 `# TODO`）
- `答案.ipynb` —— 每道题：一个 markdown cell（题目标题）+ 一个 code cell（完整答案，可直接运行）

**题目数量**：5 道，覆盖该域所有核心概念，每道考察一个知识点。

**题目内容要求**：
- 贴近学习者背景（Java 开发，Python 新手），变量名 / 场景尽量用 Java 开发者熟悉的例子
- 只考课程文件里出现过的 API，不出超纲题
- 不需要调用真实 LLM（不用 `.env`），纯本地跑通即可

**创建流程**：
1. `Write` 创建空 `.ipynb`（cells 为空数组）
2. `Read` 读取该文件（NotebookEdit 要求先 Read）
3. `NotebookEdit` 逐个 `insert` cell（先 markdown 后 code，交替插入）
