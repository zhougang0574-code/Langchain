...
💡 详细机制说明：
  1. 主链路输出脏数据："这是结果：{'name': '周星驰', 'films': ['大话西游', '功夫']}"

  2. 修复模型接收的内容包括：
     - 原始脏输出："这是结果：{'name': '周星驰', 'films': ['大话西游', '功夫']}"
     - 期望格式规范：Actor 模型定义（name: str, films: list[str]）
     - 修复指令：将脏数据转换为合法JSON格式

  3. 修复后返回干净数据：{"name": "周星驰", "films": ["大话西游", "功夫"]}

  4. 然后用干净数据重新解析成功

  🤔 优化思路：因为修复模型只负责格式化，理论上可以用一个小模型来降低成本！
"""
