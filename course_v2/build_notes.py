...
DOMAINS = [
 ...
 ("d06","06 工具与 Agent","#ef4444",[
   ...
   ("l0605","05 反思 Agent〔进阶〕","生成者⇄评审者循环：产草稿→挑毛病→据批评重写，直到 PASS 或到轮次上限。",
    "复杂任务里「自我批评再重写」常优于单次生成；停止条件(PASS/max_round)必须有，否则无限改。",
    'draft = writer.invoke({...})\ncritique = critic.invoke({"task":task,"draft":draft})\nif critique.startswith("PASS"): break   # 否则带 critique 重写'),
   ("l0606","06 路由 Agent〔进阶〕","先用 LLM(结构化输出)把请求分类，再 dispatch 到对应专家链。",
    "路由 key 由模型读懂语义后产生（区别于〔04/05〕写死的 if）；每条专家链 prompt 更聚焦省 token。",
    'route = (prompt | llm.with_structured_output(Route)).invoke({"question":q})\nexperts[route.category].invoke({"question":q})'),
   ("l0607","07 MCP 工具〔进阶〕","用 langchain-mcp-adapters 把 MCP server 的工具转成 LangChain 工具，照常 create_agent。",
    "MCP=工具接入标准、LangChain=应用框架，两层靠 adapter 桥接；Agent 侧用法零改动，变的只是工具来源。",
    'client = MultiServerMCPClient({"math":{"command":sys.executable,"args":[SERVER],"transport":"stdio"}})\ntools = await client.get_tools()\nagent = create_agent(llm, tools=tools)'),
 ]),
